...
- Player-created items (smithed, enchanted, brewed)
...
## Item Provenance Stamps

Any item the player crafts, tempers, or enchants receives a provenance
stamp — persistent metadata attached to the item itself that travels with
it wherever it goes. Inventory, barter, container, follower inventory,
chest, display case. Wherever the item appears, the stamp is on it.

The stamp is not a visual mark on the item model. It is metadata surfaced
in the item detail panel wherever that panel appears. The item looks the
same. The detail panel knows more about it.

### Stamp Data

A stamp records:
- **Action** — Crafted / Tempered / Enchanted
- **Character name** — the player character who performed the action
- **Skill level** — the relevant skill at the time of the action
- **Date** — the in-game date
- **Quality** — for tempering, the quality tier achieved
- **Dedication** — optional, see below

Displayed in the item detail panel as a provenance line:

```
Crafted by Valdris — Smithing 67 — 15th of Hearthfire, 4E 201
```
```
Enchanted by Valdris — Enchanting 82 — 22nd of Frostfall, 4E 201
```
```
Tempered by Valdris — Smithing 71 — Superior quality
```

### Accumulated History

An item is not limited to one stamp. Each action that qualifies adds a
stamp to the item's history. A weapon crafted, then tempered twice, then
enchanted carries all of that:

```
Crafted by Valdris    — Smithing 42  — 3rd of Last Seed, 4E 201
Tempered by Valdris   — Smithing 58  — 19th of Hearthfire, 4E 201
Tempered by Valdris   — Smithing 71  — 8th of Frostfall, 4E 201
Enchanted by Valdris  — Enchanting 82 — 22nd of Frostfall, 4E 201
```

This is a record of investment. A weapon the player has carried and
improved over the course of a playthrough carries visible evidence of
that journey. The history is collapsible in the detail panel — the most
recent stamp is shown by default, the full history expanded on demand.

### Dedication

At any crafting, tempering, or enchanting action, the player can optionally
dedicate the item to a named recipient. A text field — same OSK support as
the naming field — that adds a dedication line to the stamp:

```
Crafted by Valdris for Lydia — Smithing 67 — 15th of Hearthfire, 4E 201
```

The dedication is freeform text. The player types whatever they want.
OHUI does not validate it against the follower list or any other data.
It is the player's intent, recorded.

When a dedicated item is viewed in a follower's inventory through the
container screen, the dedication is prominent. "Made for Lydia" on a
chestpiece Lydia is wearing is a small thing with real weight.

### Sentimental Protection

Any stamped item is automatically eligible for a sentimental flag — a
protection layer that prevents it appearing in:
- Quick-sell and barter offer staging
- Junk tagging and bulk-sell operations
- Container Take All
- Any destructive bulk operation

The flag is opt-in per item, not automatic for all stamped items. The
player marks an item as sentimental from its context menu. A stamped item
gets a "Mark as sentimental" option that unstemped items do not. The stamp
is the eligibility gate. The player still makes the choice.

A sentimental item can still be manually sold or dropped. The protection
is against accidental loss in bulk operations, not against deliberate
decisions. The player who wants to sell their crafted sword can sell it.
The player who accidentally junk-tagged it cannot lose it to a bulk sell.

### Legacy Items

A heavily stamped item — crafted, improved multiple times, enchanted,
dedicated, marked sentimental — is a character artifact. It carries the
history of the character who made it and the journey it has been on.

This is not a system OHUI builds around. It is an emergent property of
stamps accumulating on items that matter to the player. The player who
crafts a sword at Smithing 42 and carries it to Smithing 100, tempering
it at each milestone, ends up with an item that is a record of that arc.
OHUI just makes that record visible.

### API Exposure

Stamp data is readable via OHUI's public API from both C++ and Papyrus.
The full stamp history for any item is available to any mod that wants it.

Use cases for mods reading stamp data:
- Display mods surfacing provenance on plaques or display cases
- Gift and relationship systems treating player-crafted dedicated items
  differently from bought items
- Reputation systems that care about whether a player makes their own
  equipment versus buying it
- Museum systems flagging player-crafted items for special display
- Any mod that has an opinion about whether an item was made by the player

OHUI defines the data. The ecosystem decides what to do with it.

### Integration Points

**Inventory screen** — the "player-created" filter tag surfaces all stamped
items across the inventory. Stamped items are the authoritative definition
of player-created. The filter and the stamp system are the same thing.

**Barter screen** — stamped items flagged as sentimental are excluded from
the player's offer panel by default. The protection carries into trade.

**Container screen** — dedication stamps are prominent when viewing items
in a follower's inventory. The item detail panel shows who made it for whom.

**Enchanting screen** — the naming field and the stamp are separate but
related. The player names the item and the stamp records the enchanting
action independently. Both persist. An item can have a custom name and a
provenance stamp.

**Message log / share** — a stamped item's provenance history is exportable
via the same clipboard and share infrastructure as the message log. A player
who wants to share what they made and when can do so in one action.

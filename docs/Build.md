...
|----------|-----------|---------|
| `VCPKG_ROOT` | | `C:\vcpkg` |
| `PROJECT_ROOT` | | `D:\git\ohfor\ohui-private` |
...
Output: `build\release\Release\OHUI.dll`

## Deploy

Copy the DLL to your game's SKSE plugins folder:
```cmd
copy /Y "build\release\Release\OHUI.dll" "{SKYRIM_DIR}\Data\SKSE\Plugins\"
```
...
4. Copy compiled `.pex` back to repo: `copy /Y "{SKYRIM_DIR}\Data\Scripts\OHUI_*.pex" scripts\papyrus\compiled\`
...
ESP files are created and edited in [SSEEdit](https://www.nexusmods.com/skyrimspecialedition/mods/164). They live in `Data/` in the repo. After editing in xEdit, copy the updated ESP back to the repo and commit.
